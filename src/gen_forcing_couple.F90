...
!$OMP PARALLEL DO
#if defined(__icepack_skip)
  if (mype==0 .and. init) then
     write(*,*) "Gulf of Bothnia air temperature and downward longwave cutoff if Tair <= -10C or longwave <= 250W/m2"
     init = .False.
  endif
#endif
  DO n=1, myDim_nod2D+eDim_nod2D
...
#if defined(__icepack_skip)
     ! ensure that the SST does not fall very strongly below the sea surface freezing temperature in the Gulf of Bothnia due to
     ! low ocean ventilation by cutting air temperature below -15C and downward longwave below 250W/m2 frank.kauker@awi.de 
     if (geo_coord_nod2D(1, n) >= 16.*pi/180. .and. geo_coord_nod2D(1, n) <= 27.*pi/180.) then
        if (geo_coord_nod2D(2, n) >= 60.*pi/180. .and. geo_coord_nod2D(2, n) <= 67.*pi/180.) then
           if (Tair(n)     <=  -5._WP) Tair(n)     =  -5._WP
           if (longwave(n) <= 250._WP) longwave(n) = 250._WP
           if (u_wind(n)   <=  -5._WP) u_wind(n)   =  -5._WP
           if (u_wind(n)   >=   5._WP) u_wind(n)   =   5._WP
           if (v_wind(n)   <=  -5._WP) v_wind(n)   =  -5._WP
           if (v_wind(n)   >=   5._WP) u_wind(n)   =   5._WP
        endif
     endif
#endif
