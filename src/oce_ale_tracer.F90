...
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(n, nz, nl1, ul1, vd_flux, rdata, flux, rlx, zinv1)
    ...
    do n=1, myDim_nod2D
        ...
        elseif (tracers%data(tr_num)%ID==2) then
           flux  =  virtual_salt(n) + relax_salt(n) + real_salt_flux(n) * is_nonlinfs ! sign error corrected. frank.kauker@awi.de
                                                                                      ! See https://github.com/FESOM/fesom2/pull/733
        else
            flux  = 0._WP
            rdata = 0._WP
            rlx=0._WP
  ...
  SELECT CASE (id)
    ...
    CASE (2)
        ...
        bc_surface= dt * ( virtual_salt(n) & !--> is zeros for zlevel/zstar
             + relax_salt(n) + real_salt_flux(n) * is_nonlinfs ) ! sign error corrected. frank.kauker@awi.de                
	                                                         ! See https://github.com/FESOM/fesom2/pull/733       
