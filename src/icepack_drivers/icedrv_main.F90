...
module icedrv_main
  ...
  integer (kind=int_kind), save  :: open_water_albedo ! 0=default; 1=taylor; 2=briegleb (0: constant, albocn; 1,2: zenith angle dependent)
  ...
  real (kind=dbl_kind), allocatable, save ::    & ! DIM nx; ALBEDOS AGGREGATED OVER CATEGORIES (IF CALC_TSFC)
       ...
       snowfrac(:),                             & ! snow fraction used in radiation
                                                  ! for diagnostic:
       alvdr_init(:), alidr_init(:),            & ! spectral albedos (fraction)
       alvdf_init(:), alidf_init(:),            & ! spectral albedos (fraction)
       albocn2D(:)                                ! zenith angle dependent open water albedo
  ...
  real (kind=dbl_kind), allocatable, save ::    & ! DIM nx,ncat; General pond diagnostic variables per ice category
  ...
  real (kind=dbl_kind), allocatable, save ::    & ! DIM nx: General pond diagnostic variables
  ...
  real (kind=dbl_kind), allocatable, save ::    & ! DIM nx,nslyr; SNWGRAIN DIAGNOSTIC
       rsnwavg(:,:),                            & ! average snow grain radius per layer                                                                            
       rhosavg(:,:),                            & ! average compacted snow density                                                                       
       smicetot(:,:),                           & ! total volume of ice in snow
       smliqtot(:,:)                              ! total volume of liquid in snow
  ...
  integer (kind=int_kind), save ::               &
       ! istep1,                                   & ! counter, number of steps at current timestep - unused frank.kauker@awi.de
  ...
  !=======================================================================
  ! 8. New variables introduce in Icepack.1.5.1
  !=======================================================================
