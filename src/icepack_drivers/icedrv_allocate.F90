...
contains
  ...
  subroutine alloc_state
    ...
    allocate (               &
         ...
         trcrn     (nx,max_ntrcr,ncat) , & ! ice tracers per class: 1: surface temperature of ice/snow (C)
         stat=ierr)
  ...
  subroutine alloc_flux
    ...
    allocate( &
         ...
         Tf(nx)      , & ! sea surface freezing temperature (C)
         ...
         albocn2D(nx), & ! zenith angle dependent open water albedo
         ...
         ! DIM nx,ncat; General pond diagnostic variables per ice category
         dpnd_flushn(nx,ncat),    & ! category pond flushing rate due to ice permeability                                          
         dpnd_exponn(nx,ncat),    & ! category exponential pond drainage rate                                                      
         dpnd_freebdn(nx,ncat),   & ! category pond drainage rate due to freeboard constraint                                      
         dpnd_initialn(nx,ncat),  & ! category runoff rate due to rfrac (m/step)                                                   
         dpnd_dlidn(nx,ncat),     & ! category pond loss/gain due to ice lid (m/step)                                              
         ! DIM nx: General pond diagnostic variables                                                    
         dpnd_flush(nx),          & ! pond flushing rate due to ice permeability (m/step)                                          
         dpnd_expon(nx),          & ! exponential pond drainage rate (m/step)                                                      
         dpnd_freebd(nx),         & ! pond drainage rate due to freeboard constraint (m/step)                                      
         dpnd_initial(nx),        & ! runoff rate due to rfrac (m/step)                                                            
         dpnd_dlid(nx),           & ! pond loss/gain (+/-) to ice lid freezing/melting (m/step)                                    
         dpnd_melt(nx),           & ! pond 'drainage' due to ice melting (m/step)                                                  
         dpnd_ridge(nx),          & ! pond drainage due to ridging (m/step)
         ! DIM nx: Flux variables
	 ...
	 fswthru_idr(nx),    & ! nir dir shortwave penetrating to ocean (W/m^2)
         ...
         rsnwavg(nx,nslyr),   & ! average snow grain radius per layer
         rhosavg(nx,nslyr),   & ! average compacted snow density
         smicetot(nx,nslyr),  & ! total volume of ice in snow
         smliqtot(nx,nslyr),  & ! total volume of liquid in snow
         stat=ierr)
