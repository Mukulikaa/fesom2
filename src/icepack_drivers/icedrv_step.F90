...
contains
  ...
  subroutine step_therm1 (dt)
    ...
    call icepack_query_parameters( puny_out=puny, calc_Tsfc_out=calc_Tsfc, snwgrain_out=snwgrain)
    call icepack_warnings_flush(ice_stderr)
    if (icepack_warnings_aborted()) call icedrv_system_abort(string=subname, &
         file=__FILE__,line= __LINE__)
  ...
  subroutine ocean_mixed_layer (ice, mesh, dt)
    ...
    real (kind=dbl_kind) :: albocn, coszen
    ...
    do i = 1, nx
       if(open_water_albedo > 0)then
          ! coszen=compute_solar_zenith_angle(yday, sec/3600., geo_coord_nod2D(1,i), geo_coord_nod2D(2,i))
          ! if (mype==0)  print *, 'coszen 1:', coszen
          call compute_coszen (geo_coord_nod2D(2,i), geo_coord_nod2D(1,i), yday, sec, coszen)
          coszen=max(coszen, c0)
          ! if (mype==0)  print *, 'coszen 2:', coszen
          if(open_water_albedo > 1)then
             albocn=albw_briegleb(coszen)
          else
             albocn=albw_taylor(coszen)
          endif
       endif
       ! if (mype==0)  print *, 'albocn:', yday, sec, geo_coord_nod2D(1,i), geo_coord_nod2D(2,i), coszen, albocn

       albocn2D(i) = albocn ! store albocn for diagnostics
       ...
       call ocn_mixed_layer_icepack( &
            geo_coord_nod2D(:,i),    ice,                    &
            alvdr_ocn=albocn,        swvdr=swvdr(i),         & ! For now, assume albedo = albocn in each spectral band
            alidr_ocn=albocn,        swidr=swidr(i),         & ! For now, assume albedo = albocn in each spectral band
            alvdf_ocn=albocn,        swvdf=swvdf(i),         & ! For now, assume albedo = albocn in each spectral band
            alidf_ocn=albocn,        swidf=swidf(i),         & ! For now, assume albedo = albocn in each spectral band
  ...
  subroutine ocn_mixed_layer_icepack( &
    ...
    real (kind=dbl_kind), dimension(2), intent(in) :: geo_coord_nod2D
          
    real (kind=dbl_kind), intent(in) :: &
         alvdr_ocn, alidr_ocn, alvdf_ocn, alidf_ocn, & ! visible/near-IR direct/diffusive open water albedo (fraction)
         swvdr, swvdf, swidr, swidf,                 & ! visible/near-IR direct/diffusive sw down (W/m^2)
         flw,                                        & ! incoming longwave radiation (W/m^2)
         Tf,                                         & ! sea surface freezing temperature (C)
    ...
    fhocn_tot = fhocn + fswthru                                                         & ! fluxes through sea ice; these are *aice already
         + (fsens_ocn + flat_ocn + flwout_ocn + flw + swabs + Lfresh*fsnow) * (c1-aice) & ! fluxes at open ocean
         + max( c0, frzmlt) * aice                                                        ! if sst falls below Tf, frzmlt is positive -> ocean warming ???
                                                                                          ! I still do not know what the frzmlt term is good for
                                                                                          ! frank.kauker@awi.de

    !if (sst < Tf - 0.1 .and. fhocn_tot > c0 ) then ! workaround to prevent very negative sst in the most northern part of the Gulf of Bothnia
                                                    ! with DARS2.0 mesh; frank.kauler@awi.de   
    !if (sst < Tf - 0.5) then
    !   write(*,*) 'SST is below freez. temp. (of Icepack) by more than 0.5°C', sst, Tf, fhocn_tot, mype, &
    !        geo_coord_nod2D(1)*180./3.14159, geo_coord_nod2D(2)*180./3.14159
    !   !fhocn_tot = c0
    !endif
  ...
  subroutine coupling_prep(ice, mesh, dt)
    ...
    call ocean_mixed_layer (ice, mesh, dt) ! ocean surface fluxes and sst
  ...
  module subroutine step_icepack(flag_debug, ice, mesh, time_evp, time_advec, time_therm)
    ...
    call icepack_query_parameters(skl_bgc_out=skl_bgc, z_tracers_out=z_tracers, &
         calc_Tsfc_out=calc_Tsfc, wave_spec_out=wave_spec, snwgrain_out=snwgrain)
    call icepack_query_tracer_flags(tr_brine_out=tr_brine, tr_fsd_out=tr_fsd)
    call icepack_warnings_flush(ice_stderr)
    if (icepack_warnings_aborted()) call icedrv_system_abort(string=subname, file=__FILE__,line= __LINE__)
    ...
    !-----------------------------------------------------------------
    ! calculate advanced snow module aggregated variables
    !-----------------------------------------------------------------

    if (snwgrain) then
       if (flag_debug .and. mype==0)  print *, achar(27)//'[36m'//'     --> call snow_diagnostic'//achar(27)//'[0m'
       call snow_diagnostic
    endif
    ...
    call coupling_prep (ice, mesh, dt)
  ...
  subroutine snow_diagnostic

    implicit none
    
    integer (kind=int_kind) :: k, i, n ! do loop indexes                                                                                    
    integer (kind=int_kind) :: nt_rsnw, nt_rhos, nt_smice, nt_smliq

    character(len=*), parameter :: subname='(snow_diagnostic)'

    call icepack_query_tracer_indices( &
         nt_rsnw_out=nt_rsnw, nt_rhos_out=nt_rhos, nt_smice_out=nt_smice, nt_smliq_out=nt_smliq)
    call icepack_warnings_flush(ice_stderr)
    if (icepack_warnings_aborted()) call icedrv_system_abort(string=subname, &
         file=__FILE__,line= __LINE__)
       ...
       do k = 1, nslyr
          do n = 1, ncat
             if (vsnon(i,n) > c0) then
                rsnwavg(i,k)  = rsnwavg(i,k) + trcrn(n,nt_rsnw +k-1,n) ! snow grain radius
                rhosavg(i,k)  = rhosavg(i,k) + trcrn(n,nt_rhos +k-1,n) ! compacted snow density
                smicetot(i,k) = smicetot(i,k) + trcrn(n,nt_smice+k-1,n) * vsnon(n,n)
                smliqtot(i,k) = smliqtot(i,k) + trcrn(n,nt_smliq+k-1,n) * vsnon(n,n)
             end if
          end do
          rsnwavg(i,k) = rsnwavg(i,k) / real(ncat,kind=dbl_kind) ! snow grain radius
          rhosavg(i,k) = rhosavg(i,k) / real(ncat,kind=dbl_kind) ! compacted snow density
       end do
    end do ! i

  end subroutine snow_diagnostic
  ...
  function compute_solar_zenith_angle(day_of_year, hour_utc, longitude, latitude) result(cos_zenith)
    ...
    real(kind=WP), intent(in)  :: day_of_year
