...
#include "associate_mesh.h"
          ...
          do i = 1, nx
             ...
             ! Tf < sst: frzmlt negative, i.e. ice can melt
             ! Tf > sst: frzmlt positive, i.e. ice can grow
             !if (geo_coord_nod2D(2, i) > 89.5 * 3.14159/180.) & ! calculate pole indices for analysis/debugging; frank.kauker@awi.de
          ...
          call exchange_nod(rdg_conv, rdg_shear, p_partit, luse_g2g = .true.)

          ! Clock variables
    
          days_per_year = ndpyr
          ! daymo         = num_day_in_month(fleapyear,:) - daymo is not used in Icepack columnphysics but only in the original drivers frank.kauker@awi.de
          ! if (fleapyear==1) then - daycal is not used in Icepack columnphysics but only in the original drivers frank.kauker@awi.de
          !   daycal     = daycal366
          ! else
          !    daycal     = daycal365
          ! end if
          ! istep1        = mstep - istep1 is not used in Icepack columnphysics but only in the original drivers frank.kauker@awi.de
          ! time          = mstep*dt - time is not used in Icepack columnphysics but only in the original drivers frank.kauker@awi.de
          mday          = day_in_month
          month_i       = month
          nyr           = yearnew
          sec           = int(timenew)
          yday          = real(daynew, kind=dbl_kind)
          ! dayyr         = real(days_per_year, kind=dbl_kind) - dayyr is not used in Icepack columnphysics but only in the original drivers frank.kauker@awi.de
          ! secday        = real(sec, kind=dbl_kind) - secday is defined in module icepack_parameters (icepack_parameters.F90) frank.kauker@awi.de
          calendar_type = 'GREGORIAN'
          dt_dyn        = dt/real(ndtd,kind=dbl_kind) ! dynamics et al timestep
          ! if (mype==0)  print *, 'fesom_to_icepack:', mday, sec, timenew, yday, calendar_type, dt_dyn
        ...
        module subroutine icepack_to_fesom( &
          ...
          if (present(fhocn_tot_out)         ) then
             where(sst > Tf)
                                               fhocn_tot_out    = fhocn_tot
             elsewhere
                                               fhocn_tot_out    = c0
             endwhere
          endif
