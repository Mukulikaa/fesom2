...
#include "associate_mesh.h"
    ...
    call exchange_nod(trl, p_partit, luse_g2g = .true.)
...
#include "associate_mesh.h"
    ...
    call exchange_nod(d_tr, p_partit, luse_g2g = .true.)
...
#include "associate_mesh.h"
    ...
    call exchange_nod(icepminus, icepplus, p_partit, luse_g2g = .true.)
    ...
    call exchange_nod(trc, p_partit, luse_g2g = .true.)
...
#include "associate_mesh.h"
  ...
  module subroutine tracer_advection_icepack(ice, mesh)
    ...
    logical(kind=log_kind) :: tr_pond_topo, tr_pond_lvl, tr_pond_sealvl, tr_pond, tr_aero, tr_FY, tr_iage
  ...
  subroutine work_to_state (ntrcr, narr, works)
    ...
    ! local variables
    logical (kind=log_kind) :: tr_lvl, tr_pond_lvl, tr_pond_topo, tr_pond_sealvl
    ...
    call icepack_query_tracer_flags( &
         ...
         tr_pond_sealvl_out=tr_pond_sealvl, tr_lvl_out=tr_lvl)
    call icepack_query_tracer_indices( &
    ...
    vsnon(:,:)   = c0
    aice0(:)     = c0
    !Tf(:)        = c0 ! why 'Tf' is set to zero? 'Tf' is the sea surface freezing temperature only depending on 'SSS' (and 'tfrz_option').
                       ! 'Tf' is calculated in 'subroutine fesom_to_icepack' (icedrv_transfer.F90)
  ...
  module subroutine cut_off_icepack
    ...
    logical (kind=log_kind) :: tr_brine, tr_lvl, flag_snow, flag_cold_ice, flag_warm_ice, &
         tr_pond_topo, tr_pond_lvl, tr_pond_sealvl, tr_FY, tr_iage
       ...
       do i = 1, nx ! For each grid point
          ...
          if (vicen(i,n) > small .and. aicen(i,n) > small) then
             ...
             do k = 1, nilyr  ! Check for problematic salinities - frank-kauler@awi.de
                ...
                if (trcrn(i,nt_sice+k-1,n) > c100) trcrn(i,nt_sice+k-1,n) = c100
             enddo
    ...
    do n = 1, ncat
       do i = 1, nx
          if (aicen(i,n) > c0) then
             ...
             if (tr_pond_topo .or. tr_pond_lvl .or. tr_pond_sealvl) then
