...
contains
  ...
  subroutine init_state(tracer)
    ...
    logical (kind=log_kind) :: &
         tr_iage, tr_FY, tr_lvl, tr_aero, tr_fsd, tr_iso, tr_snow, tr_pond_lvl, tr_pond_topo, tr_pond_sealvl
    integer (kind=int_kind) :: &
  ...
  subroutine init_coupler_flux()
    ...
    do i = 1, nx
       Tf(i) = icepack_sea_freezing_temperature(sss(i)) ! sea surface freezing temp - deg C
    enddo
    !write(*,*) 'icedrv_init: min/maxval (Tf, sss)=', minval(Tf), maxval(Tf), minval(sss), maxval(sss)
  ...
  subroutine init_shortwave()
    ...
    if (trim(shortwave) == 'dEdd') then ! delta Eddington
       ...
       ! if (icepack_warnings_aborted()) call icedrv_system_abort(i, istep1, subname, __FILE__,__LINE__) - istep1 is unused frank.kauker@awi.de
...
!=======================================================================
  ...
  subroutine init_state_var (tracer)
    ...
    !if (mype==0) write(nu_diag,*) 'icedrv.init_state_var: Max/Min ocn. surf. freezing temperature = ', maxval(Tf), minval(Tf)
